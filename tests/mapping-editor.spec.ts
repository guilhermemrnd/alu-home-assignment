...
test.describe('Mapping Editing', () => {
  ...
  test('should allow editing mappings and update products', async ({ page }) => {
    // Check initial mapping (assuming AI infers correctly) 
    await expect(page.locator('span').filter({ hasText: 'Purchase Order' })).toBeVisible();

    // Change a mapping
    await page.locator('[role="combobox"]').filter({ hasText: 'Product Name' }).click();
    await page.locator('[role="option"]').filter({ hasText: 'Country of Origin' }).click();

    // Check if products update (product name should now show country)
    await expect(page.locator('td:has-text("USA")').first()).toBeVisible();
  });
});